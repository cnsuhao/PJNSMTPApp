...
{
  CPJNSMTPConnection cConnection;
  if (UpdateData(TRUE))
  {
    ...
    else 
    {
      ...
      try
      {
        ...
        if (m_bDNSLookup)
        {
          ...
          else
          {
            ...
            #ifdef CPJNSMTP_MFC_EXTENSIONS
              if (!cConnection.MXLookup(sDomain, servers, priorities))
            #else
              if (!cConnection.MXLookup(sDomain.c_str(), servers, priorities))
            #endif //#ifdef CPJNSMTP_MFC_EXTENSIONS
              {
                CString sMsg;
              #ifdef CPJNSMTP_MFC_EXTENSIONS
        ...
        if (bSend)
        {
          ...
          if (m_bAutoDial)
            cConnection.ConnectToInternet();

          //Connect and send the message
          cConnection.SetSSLProtocol(m_SSLProtocol);
          cConnection.SetBindAddress(m_sBoundIP);
          cConnection.SetInitialClientResponse(m_bInitialClientResponse);
        #ifdef CPJNSMTP_MFC_EXTENSIONS
          cConnection.Connect(sHost, m_Auth, m_sUsername, m_sPassword, m_nPort, m_ConnectionType, pMessage->m_dwDSN);
        #else
          cConnection.Connect(sHost.c_str(), m_Auth, m_sUsername, m_sPassword, m_nPort, m_ConnectionType, pMessage->m_dwDSN);
        #endif //#ifdef CPJNSMTP_MFC_EXTENSIONS
          cConnection.SendMessage(*pMessage);

          //Auto disconnect from the internet
          if (m_bAutoDial)
            cConnection.CloseInternetConnection();
        }
      }
    #ifdef CPJNSMTP_MFC_EXTENSIONS
      ...
      try
      {
        //Disconnect if already connected
        if (cConnection.IsConnected())
          cConnection.Disconnect(bCloseGracefully);
      }
    #ifdef CPJNSMTP_MFC_EXTENSIONS
      catch(CPJNSMTPException* pEx)
      {
