...
History: PJN / 22-10-2014 1. Initial public release
         ...
         PJN / 03-02-2017 1. Updated copyright details.
                          2. Removed unnecessary demo code from the demo app included in the download. As 
                          reported by Geoff Stevens, this demo code which calls the ExportSecurityContext and
                          FreeContextBuffer APIs seems to cause heap corruption inside the FreeContextBuffer
                          call. The issue only seems to occur if the FreeContextBuffer call occurs very 
                          quickly after the ExportSecurityContext call. If the call to FreeContextBuffer 
                          occurs more than 100 ms later than ExportSecurityContext then the heap corruption 
                          does not occur. I was not able to reproduce the issue and as the code was just
                          demo code, I have simply removed it from the demo app.
                          3. Fixed a number of typos in the text when calling the Audit methods.
         PJN / 15-04-2017 1. Updated the sample app to log more information about the connection in the 
                          DisplayConnectionInfo function. New details logged include SECPKGCONTEXT_CONNECTION_INFO_EX_V1,
                          SECPKG_ATTR_CIPHER_INFO, SECPKG_ATTR_CIPHER_STRENGTHS, SECPKG_ATTR_SUPPORTED_ALGS, 
                          SECPKG_ATTR_SUPPORTED_PROTOCOLS, SECPKG_ATTR_APP_DATA, SECPKG_ATTR_AUTHORITY, 
                          SECPKG_ATTR_EAP_KEY_BLOCK, SECPKG_ATTR_FLAGS, SECPKG_ATTR_LIFESPAN,
                          SECPKG_ATTR_NAMES, SECPKG_ATTR_SESSION_INFO, SECPKG_ATTR_SESSION_KEY,
                          SECPKG_ATTR_SIZES, SECPKG_ATTR_STREAM_SIZES, SECPKG_ATTR_SUPPORTED_SIGNATURES, 
                          SECPKG_ATTR_TARGET_INFORMATION, SECPKG_ATTR_KEYING_MATERIAL, SECPKG_ATTR_SRTP_PARAMETERS,
                          SECPKG_ATTR_TOKEN_BINDING & SECPKG_ATTR_KEYING_MATERIAL_TOKEN_BINDING attributes. This also 
                          includes provisional logging of TLS v1.3 details. At the moment Windows 10 v1703 does not 
                          support TLS v1.3 but I would expect Windows 10 to include support for this protocol sometime 
                          in 2017.
                          2. Reviewed schannel.h header file from Windows 10 v15063 SDK for any changes to SChannel
                          in Windows 10 v1703.
...
#pragma comment(lib, "Secur32.lib")  //Automatically link in the Security dll
...
#pragma message("To avoid this message, please put schannel.h in your pre compiled header (normally stdafx.h)")
#include <schannel.h>
#endif //#ifndef __SCHANNEL_H__
...
#include "CryptoWrappersCertificate.h"  //If you get a compilation error about this missing header file, then you need to download my CryptoWrappers wrapper classes from http://www.naughter.com/cryptowrappers.html
#include "CryptoWrappersChain.h"  //If you get a compilation error about this missing header file, then you need to download my CryptoWrappers wrapper classes from http://www.naughter.com/cryptowrappers.html
#include "CryptoWrappersStore.h"  //If you get a compilation error about this missing header file, then you need to download my CryptoWrappers wrapper classes from http://www.naughter.com/cryptowrappers.html
...
//Methods
  ...
  SECURITY_STATUS Acquire(_In_ unsigned long fCredentialUse, _In_opt_ SCHANNEL_CRED* pAuthData, _Out_opt_ PTimeStamp ptsExpiry = nullptr)
  {
    ...
    return AcquireCredentialsHandle(nullptr, const_cast<LPTSTR>(UNISP_NAME), fCredentialUse, nullptr, pAuthData, nullptr, nullptr, &m_Handle, ptsExpiry);
  }
...
//Methods
  ...
  SECURITY_STATUS Import(_In_ PSecBuffer pPackedContext, _In_ VOID* Token)
  {
    ...
    return ImportSecurityContext(const_cast<LPTSTR>(UNISP_NAME), pPackedContext, Token, &m_Handle);
  }
...
//Methods
  SECURITY_STATUS AcquireClient(_In_opt_ LPTSTR pszClientCertificateName = nullptr, _In_ LPCSTR lpszStoreProvider = CERT_STORE_PROV_SYSTEM, _In_ DWORD dwCertOpenStoreFlags = CERT_SYSTEM_STORE_CURRENT_USER | CERT_STORE_READONLY_FLAG, _In_opt_ const void* pvCertOpenStorePara = L"MY")
  {
    ...
    if (pszClientCertificateName != nullptr)
    {
      ...
      certRDNAttr[0].pszObjId = const_cast<LPSTR>(szOID_COMMON_NAME);
...
//Constructors / Destructors
           ...
           m_lExtraReadData(0)
  {
    ...
    m_LastStatus = QuerySecurityPackageInfo(const_cast<LPTSTR>(UNISP_NAME), &pSecInfo);
...
//Non virtual methods
  ...
  virtual SECURITY_STATUS SSLConnect(_In_ LPCTSTR pszServerName)
  {
    ...
    if (OutBuffers[0].cbBuffer != 0)
    {
      ...
      {
        if (m_dwAuditFlags & AUDIT_ERRORS)  
          Audit(_T("CSSL::SSLConnect, An error occurred sending data to the server, Error:0x%08X\n"), status);
      }
    }
  ...
  virtual SECURITY_STATUS SSLRequestRenegotiationClient(_In_ LPCTSTR pszServerName)
  {
    ...
    if (OutBuffers[0].cbBuffer != 0)
    {
      ...
      {
        if (m_dwAuditFlags & AUDIT_ERRORS)  
          Audit(_T("CSSL::SSLRequestRenegotiationClient, An error occurred sending data to the server, Error:0x%08X\n"), status);
      }
    }
  ...
  virtual SECURITY_STATUS SSLRequestRenegotiationServer(_In_ BOOL bClientAuth)
  {
    ...
    if (OutBuffers[0].cbBuffer != 0)
    {
      ...
      {
        if (m_dwAuditFlags & AUDIT_ERRORS)  
          Audit(_T("CSSL::SSLRequestRenegotiationServer, An error occurred sending data to the client, Error:0x%08X\n"), status);
      }
    }
      ...
      if (status == SEC_E_INCOMPLETE_MESSAGE)
      {
        ...
        if (SUCCEEDED(status2))
        {
          ...
          else
          {
            //Do auditing if necessary
            if (m_dwAuditFlags & AUDIT_ERRORS)
              Audit(_T("CSSL::GetEncryptedMessage, A graceful disconnect occurred reading data\n"));
  ...
  virtual SECURITY_STATUS ServerHandshakeLoop(_In_ unsigned long nContextRequest, _In_ BOOL bDoInitialRead)
  {
    ...
    do
    {
      ...
      if ((status == SEC_E_INCOMPLETE_MESSAGE) || (status == SEC_I_CONTINUE_NEEDED))
      {
        ...
        {
          if (m_dwAuditFlags & AUDIT_ERRORS)
            Audit(_T("CSSL::ServerHandshakeLoop, An error occurred reading data from the client, Error:0x%08X\n"), status2);
        ...
        {
          if (m_dwAuditFlags & AUDIT_ERRORS)
            Audit(_T("CSSL::ServerHandshakeLoop, A graceful disconnect occurred reading data from the client\n"));
      ...
      {  
        if (OutBuffers[0].cbBuffer != 0)
        {
          ...
          {
            if (m_dwAuditFlags & AUDIT_ERRORS)  
              Audit(_T("CSSL::ServerHandshakeLoop, An error occurred sending data to the client, Error:0x%08X\n"), status2);
    ...
    do
    {
      ...
      if ((status == SEC_E_INCOMPLETE_MESSAGE) || (status == SEC_I_CONTINUE_NEEDED))
      {
        ...
        if (status2 != SEC_E_OK)
        {
          //Do auditing if necessary
          if (m_dwAuditFlags & AUDIT_ERRORS)
            Audit(_T("CSSL::ClientHandshakeLoop, An error occurred reading data from the server, Error:0x%08X\n"), status2);
        ...
        if (lReceived == 0)
        {
          //Do auditing if necessary
          if (m_dwAuditFlags & AUDIT_ERRORS)
            Audit(_T("CSSL::ClientHandshakeLoop, A graceful disconnect occurred reading data from the server\n"));
      ...
      {  
        if (OutBuffers[0].cbBuffer != 0)
        {
          ...
          {
            if (m_dwAuditFlags & AUDIT_ERRORS)  
              Audit(_T("CSSL::ClientHandshakeLoop, An error occurred sending data to the server, Error:0x%08X\n"), status2);
